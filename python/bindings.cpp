...
NB_MODULE(Scribe, m) {
    ...
    nb::class_<Scribe::BytePairEncoder>(m, "BytePairEncoder", "A module that applies BPE algorithm on the given text to generate word/sub-word tokens."
        ...
        "   normalizedRanking (bool): If false, follows default BPE Scoring for token pairs, i.e. P(A|B). If true, follows WordPiece Scoring for token pairs, i.e. P(A|B)/P(A)*P(B)."
